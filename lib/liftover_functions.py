    ...
    for line in reader(chain_file):
        ...
        if fields[0] == 'chain' and nfields in [12, 13]:
            ...
            if this_chain['tName'] not in target_contig_list:
                skip = True
                if this_chain['tName'].find("alt") == -1:
                  if this_chain['tName'].find("fix") == -1:
            ...
            if this_chain['qName'] not in query_contig_list:
                skip = True
                if this_chain['qName'].find("alt") == -1:
                  if this_chain['qName'].find("fix") == -1:
...
def process_chunk_se(i, infile = None, 
                     outfile_prefix = None, 
                     old_header = None, 
                     new_header = None, 
                     target_fasta = None, 
    ...
    #print(old_header)
    outfile_prefix = outfile_prefix + "." + str(i)
    
    SAMFILE = pysam.AlignmentFile(infile, "rb", threads=1)
...
def process_chunk_pe(i, infile = None, 
                     outfile_prefix = None, 
                     old_header = None, 
                     new_header = None, 
                     target_fasta = None, 
